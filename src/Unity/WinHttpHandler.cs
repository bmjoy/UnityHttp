...
namespace UnityHttp.Net.Http
{
    ...
    internal class WinHttpHandler : HttpMessageHandler
    {
            ...
            set
            {
                if (value <= 0)
                {
                    throw new ArgumentOutOfRangeException(
                        nameof(value),
                        value,
                        "net_http_value_must_be_greater_than_zero");
                }
            ...
            CancellationToken cancellationToken)
        {
            if (request == null)
            {
                throw new ArgumentNullException(nameof(request), "net_http_handler_norequest");
            }

            // Check for invalid combinations of properties.
            if (_proxy != null && _windowsProxyUsePolicy != WindowsProxyUsePolicy.UseCustomProxy)
            {
                throw new InvalidOperationException("net_http_invalid_proxyusepolicy");
            }

            if (_windowsProxyUsePolicy == WindowsProxyUsePolicy.UseCustomProxy && _proxy == null)
            {
                throw new InvalidOperationException("net_http_invalid_proxy");
            }

            if (_cookieUsePolicy == CookieUsePolicy.UseSpecifiedCookieContainer &&
                _cookieContainer == null)
            {
                throw new InvalidOperationException("net_http_invalid_cookiecontainer");
            }
        ...
        private static bool IsChunkedModeForSend(HttpRequestMessage requestMessage)
        {
            ...
            {
                throw new InvalidOperationException("net_http_chunked_not_allowed_with_empty_content");
            }
        ...
        private void HandleAsyncException(WinHttpRequestState state, Exception ex)
        {
            ...
            else if (ex is WinHttpException || ex is IOException)
            {
                ...
                state.Tcs.TrySetException(new HttpRequestException("net_http_client_execution_error", ex));
            }
            else
            {
