...
namespace UnityHttp.Net.Http.Headers
{
    public class WarningHeaderValue : ICloneable
    {
        ...
        private static void CheckAgent(string agent)
        {
            ...
            {
                throw new ArgumentException("net_http_argument_empty_string", nameof(agent));
            }
            ...
            {
                throw new FormatException(string.Format(System.Globalization.CultureInfo.InvariantCulture, "net_http_headers_invalid_value: {0}", agent));
            }
        }
    }
}
