    ...
    internal static class HeaderUtilities
    {
        ...
        internal static double? GetQuality(ObjectCollection<NameValueHeaderValue> parameters)
        {
            ...
            if (qualityParameter != null)
            {
                ...
                UnityEngine.Debug.LogError(string.Format(System.Globalization.CultureInfo.InvariantCulture, "net_http_log_headers_invalid_quality: {0}", qualityParameter.Value));
            }
            return null;
        }
        ...
        internal static void CheckValidToken(string value, string parameterName)
        {
            ...
            {
                throw new ArgumentException("net_http_argument_empty_string", parameterName);
            }

            if (HttpRuleParser.GetTokenLength(value, 0) != value.Length)
            {
                throw new FormatException(string.Format(System.Globalization.CultureInfo.InvariantCulture, "net_http_headers_invalid_value: {0}", value));
            }
        }
        ...
        internal static bool IsValidEmailAddress(string value)
        {
            ...
            {
                UnityEngine.Debug.LogError(string.Format(System.Globalization.CultureInfo.InvariantCulture, "net_http_log_headers_wrong_email_format: {0} - {1}", value, e.Message));
            }
            return false;
        }
