...
using UnityHttp.Net.Http.Headers;
    ...
    public abstract class HttpContent : IDisposable
    {
        ...
        public Task LoadIntoBufferAsync(long maxBufferSize)
        {
            ...
            if (maxBufferSize > HttpContent.MaxBufferSize)
            {
                ...
                throw new ArgumentOutOfRangeException(nameof(maxBufferSize), maxBufferSize,
                    ...
                    "net_http_content_buffersize_limit: {0}", HttpContent.MaxBufferSize));
            }
        ...
        private MemoryStream CreateMemoryStream(long maxBufferSize, out Exception error)
        {
            ...
            if (contentLength != null)
            {
                ...
                {
                    error = new HttpRequestException(string.Format(System.Globalization.CultureInfo.InvariantCulture, "net_http_content_buffersize_exceeded: {0}", maxBufferSize));
                    return null;
                }
        ...
        private void CheckTaskNotNull(Task task)
        {
            if (task == null)
            {
                throw new InvalidOperationException("net_http_content_no_task_returned");
            }
        }
        ...
        private static Exception GetStreamCopyException(Exception originalException)
        {
            ...
            return StreamCopyExceptionNeedsWrapping(originalException) ?
                new HttpRequestException("net_http_content_stream_copy_error", originalException) :
                originalException;
        }
        ...
        internal sealed class LimitMemoryStream : MemoryStream
        {
            ...
            private void CheckSize(int countToAdd)
            {
                ...
                {
                    throw new HttpRequestException(string.Format(System.Globalization.CultureInfo.InvariantCulture, "net_http_content_buffersize_exceeded: {0}", _maxSize));
                }
            }
        }
    }
