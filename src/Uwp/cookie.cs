...
namespace UnityHttp.Net.Internal
{
    ...
    public sealed class Cookie
    {
        ...
        public string Name
        {
            ...
            set
            {
                ...
                {
                    throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", "Name", value == null ? "<null>" : value));
                }
            }
        }
        ...
        internal bool VerifySetDefaults(CookieVariant variant, Uri uri, bool isLocalDomain, string localDomain, bool set_default, bool isThrow)
        {
            ...
            if (m_name == null || m_name.Length == 0 || m_name[0] == '$' || m_name.IndexOfAny(Reserved2Name) != -1)
            {
                if (isThrow)
                {
                    throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", "Name", m_name == null ? "<null>" : m_name));
                }
                return false;
            }
                ...
                (!(m_value.Length > 2 && m_value[0] == '\"' && m_value[m_value.Length - 1] == '\"') && m_value.IndexOfAny(Reserved2Value) != -1))
            {
                if (isThrow)
                {
                    throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", "Value", m_value == null ? "<null>" : m_value));
                }
                return false;
            }
            ...
            {
                if (isThrow)
                    throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", CommentAttributeName, Comment));
                return false;
            }
                ...
                && (Path.IndexOfAny(Reserved2Value) != -1))
            {
                if (isThrow)
                {
                    throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", PathAttributeName, Path));
                }
                return false;
            }
            ...
            else
            {
                if (!m_domain_implicit)
                {
                    ...
                    if (!DomainCharsTest(domain))
                    {
                        if (isThrow)
                        {
                            throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", DomainAttributeName, domain == null ? "<null>" : domain));
                        }
                        return false;
                    }
                    ...
                    if (domain[0] != '.')
                    {
                        if (!(variant == CookieVariant.Rfc2965 || variant == CookieVariant.Plain))
                        {
                            if (isThrow)
                            {
                                throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", DomainAttributeName, m_domain));
                            }
                            return false;
                        }
                        domain = '.' + domain;
            ...
            else
            {
                ...
                if (!path.StartsWith(CookieParser.CheckQuoted(m_path)))
                {
                    if (isThrow)
                    {
                        throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", PathAttributeName, m_path));
                    }
                    return false;
                }
            }
            ...
            if (m_port_implicit == false)
            {
                ...
                if (!valid)
                {
                    if (isThrow)
                    {
                        throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", PortAttributeName, m_port));
                    }
                    return false;
                }
            }
        ...
        public string Port
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        throw new CookieException(string.Format("net_cookie_attribute: {0} - {1}", PortAttributeName, value));
                    }
